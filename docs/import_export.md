...
attachment. Both formats include stable UUIDs for locations, items, holdings, and location
relations.
...
CSV uses one table with a `record_type` column. Fields that are structured in the domain model,
such as aliases, metadata, and attributes, contain compact JSON values.
